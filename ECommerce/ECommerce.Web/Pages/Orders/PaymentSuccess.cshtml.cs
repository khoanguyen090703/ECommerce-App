using System.Net;
using System.Text.Json;
using ECommerce.SharedViewModels.DTOs.Response;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc.RazorPages;

namespace ECommerce.Web.Pages.Orders;

[Authorize]
public class PaymentSuccessModel : PageModel
{
    private static readonly TimeSpan OrderApiTimeout = TimeSpan.FromSeconds(8);

    private readonly IHttpClientFactory _httpClientFactory;

    private static readonly JsonSerializerOptions JsonOptions = new()
    {
        PropertyNameCaseInsensitive = true
    };

    public PaymentSuccessModel(IHttpClientFactory httpClientFactory)
    {
        _httpClientFactory = httpClientFactory;
    }

    public string? SessionId { get; private set; }

    public int? OrderId { get; private set; }

    public string? PageError { get; private set; }

    public bool OrderLoaded { get; private set; }

    public string? PaymentStatus { get; private set; }

    public decimal? TotalAmount { get; private set; }

    public bool ReturnedFromStripe { get; private set; }

    public bool OrderDetailsUnavailable { get; private set; }

    public bool ShouldSyncPaymentInBackground =>
        OrderLoaded
        && !string.Equals(PaymentStatus, "Paid", StringComparison.OrdinalIgnoreCase);

    public async Task OnGetAsync(CancellationToken cancellationToken = default)
    {
        SessionId = Request.Query["session_id"].FirstOrDefault();
        ReturnedFromStripe = !string.IsNullOrWhiteSpace(SessionId);

        if (!int.TryParse(Request.Query["orderId"].FirstOrDefault(), out var id) || id < 1)
        {
            PageError = "Thiếu mã đơn hàng trên đường dẫn. Vui lòng vào mục Đơn hàng của tôi để xem trạng thái thanh toán.";
            return;
        }

        OrderId = id;

        if (ReturnedFromStripe)
            OrderLoaded = true;

        using var timeoutCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        timeoutCts.CancelAfter(OrderApiTimeout);

        try
        {
            var client = _httpClientFactory.CreateClient();
            using var response = await client.GetAsync($"api/orders/{id}", timeoutCts.Token);

            if (response.StatusCode == HttpStatusCode.Unauthorized)
            {
                if (ReturnedFromStripe)
                {
                    OrderDetailsUnavailable = true;
                    return;
                }

                PageError = "Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại để xem đơn hàng.";
                return;
            }

            if (response.StatusCode == HttpStatusCode.NotFound || response.StatusCode == HttpStatusCode.Forbidden)
            {
                if (ReturnedFromStripe)
                {
                    OrderDetailsUnavailable = true;
                    return;
                }

                PageError = "Không tìm thấy đơn hàng hoặc bạn không có quyền xem đơn này.";
                return;
            }

            if (!response.IsSuccessStatusCode)
            {
                if (ReturnedFromStripe)
                {
                    OrderDetailsUnavailable = true;
                    return;
                }

                PageError = "Không tải được thông tin đơn hàng. Vui lòng thử lại sau hoặc xem trong mục Đơn hàng của tôi.";
                return;
            }

            var json = await response.Content.ReadAsStringAsync(timeoutCts.Token);
            var order = JsonSerializer.Deserialize<OrderDetailsResponse>(json, JsonOptions);
            if (order == null)
            {
                if (ReturnedFromStripe)
                {
                    OrderDetailsUnavailable = true;
                    return;
                }

                PageError = "Không đọc được dữ liệu đơn hàng.";
                return;
            }

            OrderLoaded = true;
            PaymentStatus = order.PaymentStatus;
            TotalAmount = order.TotalAmount;
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            if (ReturnedFromStripe)
            {
                OrderDetailsUnavailable = true;
                return;
            }

            PageError = "Không tải được thông tin đơn hàng kịp thời. Vui lòng mở lại từ mục Đơn hàng của tôi.";
        }
        catch (HttpRequestException)
        {
            if (ReturnedFromStripe)
            {
                OrderDetailsUnavailable = true;
                return;
            }

            PageError = "Không kết nối được máy chủ đơn hàng. Vui lòng thử lại sau.";
        }
    }
}
